...
int main()
{
  ...
  sessions[23]->getCurrentState()->processAuthResult(sessions[23], 0); // Valid call in this state
  cout << "Printing state name after state change (expecting to change to closed state)..." << endl;
  sessions[23]->getCurrentState()->printStateName();  
  ...
  // changed the state of the Session object to ClosedState.
